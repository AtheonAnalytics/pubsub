...
class Worker(ConsumerMixin):
    ...
    def get_consumers(self, Consumer, channel):
        return [
            Consumer(queues=self.queues, accept=["json"], callbacks=[self.process_task])
        ]

    def process_task(self, body, message):
        routing_key = message.delivery_info["routing_key"]
        logger.info(
            "Recieved event", extra={"routing_key": routing_key, "message_body": body}
        )
        try:
            ...
            logger.exception(
                ...
                extra={"routing_key": routing_key, "message_body": body},
            )
    ...
    def queue_task(self, routing_key, body):
        ...
        for task_mapping in task_mappings:
            ...
            celery_kwargs = (
                get_celery_kwargs(routing_key, body) if get_celery_kwargs else {}
            )
            task_result = celery_app.send_task(
                task_name, args=task_args, kwargs=task_kwargs, **celery_kwargs
            )
            results.append(task_result)

            logger.info(
                "Task queued", extra={"task_id": task_result.id, "task_name": task_name}
            )
        return results
